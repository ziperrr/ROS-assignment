...
turtle_followerA.name ="followerA"
...
#pub_control_fa	=rospy.Publisher("followerA/cmd_vel",Twist,queue_size=10)
...
def leader_callback(data):
	...
	if data.instructionID ==1:
	    finish=1
	elif data.instructionID ==0 and data.message.split(':')[0] == "f1" and sd==0:
	    sd =1
	    tmp = Turtle()
		...
		continue
  	    print("followerA " +str(tmp.x)+" "+str(tmp.y))
	    ret=follow2goal('leader','followerA',0.5)
...
if __name__ =="__main__":
    ...
    end_pub=rospy.Publisher("/leader/end",End,queue_size=10)
    while True:
        if finish ==1:
            finish =0
	    ft=1
